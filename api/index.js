...
const handleAIRequest = async (req, res) => {
  try {
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      fetchResponse = await fetch('https://ai.hackclub.com/proxy/v1/chat/completions', {
        method: 'POST',
        headers: {
...
// Error handler for JWT and other midleware
app.use((err, req, res, next) => {
  if (err.name === 'UnauthorizedError' || err.status === 401 || err.status === 403) {
    console.error('Auth Error:', err.message, '| Path:', req.path);
